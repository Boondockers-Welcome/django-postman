...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if verbose >= 1:
            self.stdout.write(
                datetime.now().strftime("%H:%M:%S ") + "Checking messages and conversations for inconsistencies..."
            )
        checks = [
            ...
            ("The message cannot be replied without having been read.", Q(
                eplied_at__isnull=False, read_at__isnull=True
            )),
        ...
        if verbose >= 1:
            self.stdout.write(datetime.now().strftime("%H:%M:%S ") + (
                "Number of inconsistencies found: {0}. See details on the error stream.".format(
                    count
                ) if count else "All is correct."
            ))
